...
/**
 * <p>
 * GSModelInteriorDescriptorStructureTests class.
 * </p>
 *
 */
public class GSModelInteriorDescriptorStructureTests extends Capability1Tests {

	/** Constant <code>DATASET_CODE="307"</code> */
	public static final String DATASET_CODE = "307";

	/** Constant <code>DATASET_DIRECTORY="307_GSModelInteriorDescriptor"</code> */
	public static final String DATASET_DIRECTORY = "307_GSModelInteriorDescriptor";

	/**
	 * A String Array holds the allowed file extensions for archives, and the Set is used
	 * for searching/matching.
	 */
	protected static final String[] ALLOWED_ARCHIVE_EXT = new String[] { "zip" };

	/** Constant <code>ALLOWED_ARCHIVE_EXT_SET</code> */
	protected static final Set<String> ALLOWED_ARCHIVE_EXT_SET = new HashSet<String>(
			Arrays.asList(ALLOWED_ARCHIVE_EXT));

	/**
	 * A String Array holds the allowed file extensions for archive entry files, and the
	 * Set is used for searching/matching.
	 */
	protected static final String[] ALLOWED_ENTRY_EXT = new String[] { "xml" };

	/** Constant <code>ALLOWED_ENTRY_EXT_SET</code> */
	protected static final Set<String> ALLOWED_ENTRY_EXT_SET = new HashSet<String>(Arrays.asList(ALLOWED_ENTRY_EXT));

	/**
	 * Validates that GSModelInteriorDescriptor filenames have valid codes/names.
	 * @throws java.io.IOException Error reading from CDB
	 */
	@Test(description = "OGC 15-113r3, A.1.13, Test 40")
	public void verifyGSModelInteriorDescriptorFile() throws IOException {
		...
		// Skip test if CDB does not have a GSModelInteriorDescriptor directory.
		if (datasetPaths.isEmpty()) {
			throw new SkipException("No GSModelInteriorDescriptor present; test skipped.");
		}
		...
		Pattern filePattern = Pattern.compile(FilenamePatterns.GSModelInteriorDescriptor);
	...
	/**
	 * Validates that GSModelInteriorDescriptor files are ZIP archive files with no
	 * compression, and limited to 32 MB.
	 * @throws java.io.IOException Error reading from CDB
	 */
	@Test(description = "OGC 15-113r3, Section 3.6.3.2")
	public void verifyGSModelInteriorDescriptorFileArchive() throws IOException {
		// Collect all matching datasets in the Tiles directory
		ArrayList<Path> datasetPaths = getDatasetPaths(this.path, DATASET_DIRECTORY);
		...
		iterateDatasets(datasetPaths, (archive -> {
			String filename = archive.getFileName().toString();
			Matcher match = filePattern.matcher(filename);
			// Any files that do not match the GSModelInteriorDescriptor file pattern will
			// be ignored, and will fail "verifyGSModelInteriorDescriptorFile()" instead.
			if (match.find()) {
				...
				try {
					...
					while (entries.hasMoreElements()) {
						...
						if (entry.getMethod() != ZipEntry.STORED) {
							errors.add(String.format("Entry '%s' in ZIP archive '%s' should not be compressed",
	...
	/**
	 * Validates that GSModelInteriorDescriptor entries inside ZIP archive follow naming
	 * conventions.
	 * @throws java.io.IOException Error reading from CDB
	 */
	@Test(description = "OGC 15-113r3, A.1.13, Test 72")
	public void verifyGSModelInteriorDescriptorEntry() throws IOException {
		// Collect all matching datasets in the Tiles directory
		ArrayList<Path> datasetPaths = getDatasetPaths(this.path, DATASET_DIRECTORY);
		...
		ArrayList<String> errors = new ArrayList<String>();
		Pattern archiveNamePattern = Pattern.compile(FilenamePatterns.GSModelInteriorDescriptor);
		Pattern archiveEntryPattern = Pattern.compile(FilenamePatterns.GSModelInteriorDescriptorEntry);
			...
			if (match.find()) {
				...
				try {
					...
					while (entries.hasMoreElements()) {
						...
						if (!entryMatch.find()) {
							errors
								.add(String.format("Invalid entry '%s' in ZIP archive '%s'", entryFilename, filename));
						}
						else {
							// groups: lat, lon, datasetCode, cs1, cs2, lod, uref, rref,
							// featureCode, fsc, modl, ext
							...
							if (!datasetsValidator.isValidCode(datasetCode)) {
								errors.add(String.format("Invalid code %s in entry '%s'",
										entryMatch.group("datasetCode"), entryFilename));
							}
