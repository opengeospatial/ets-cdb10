...
public class DefaultsXmlStructureTests extends Capability2Tests {
	...
	private ArrayList<String> collectNameNodesWithDatasetValue(String datasetValue) {
		NodeList nameNodes = XMLUtils.getNodeList("//Default_Value[Dataset/text() = \"" + datasetValue + "\"]/Name",
				defaults.getXmlFilePath());
	...
	/**
	 * <p>
	 * verifyDefaultsXmlAgainstSchema.
	 * </p>
	...
	public void verifyDefaultsXmlAgainstSchema() throws IOException, SAXException {
		...
		Assert.assertEquals(errors, "",
				defaults.getXmlFileName() + " does not validate against its XML Schema file. Errors: " + errors);
	}

	/**
	 * <p>
	 * verifyDefaultsXmlElementR_W_TypeHasValidValues.
	 * </p>
	 */
	...
	public void verifyDefaultsXmlElementR_W_TypeHasValidValues() {
		...
		for (String value : values) {
			Assert.assertTrue(VALID_VALUES.contains(value), String.format(
					"Defaults.xml element R_W_Type should " + "have a value of R or W. Value '%s' is not valid.",
					value));
		}
	}
	...
	/**
	 * <p>
	 * verifyDefaultsXmlNameIsUniqueForEachDataset.
	 * </p>
	 */
	@Test(description = "OGC 15-113r3, A.1.19, Test 77")
	public void verifyDefaultsXmlNameIsUniqueForEachDataset() {
		...
		for (String datasetValue : datasetValues) {
			...
			for (String name : names) {
				Assert.assertEquals(Collections.frequency(names, name), 1, String.format(
						"Defaults.xml element Name should be " + "unique under each Dataset. '%s' is not unique.",
						name));
			}
		}
	}
	...
	/**
	 * <p>
	 * verifyDefaultsXmlElementTypeHasValidValue.
	 * </p>
	 */
	@Test(description = "OGC 15-113r3, A.1.19, Test 77")
